...
# ----------------------------
# NEW: correlation-spike settings
# ----------------------------
CORR_WIN = 63       # ~3 months rolling corr
HIST_WIN = 256    # ~1y rolling history for spike threshold
SPIKE_Q  = 0.90      # top 10% = spike (robust across regimes)
...
# =========================
emaL  = qqq.ewm(span=180, adjust=False).mean()
ema10 = qqq.ewm(span=40,  adjust=False).mean()

exit_margin = 1.02  # exit bear only if QQQ > 1.02 * EMA180

bear_on  = (qqq < emaL)
bear_off = (qqq > emaL * exit_margin)
...
risk3 = spike_flag(corr_spy_tlt, win=HIST_WIN, q=SPIKE_Q) | spike_flag(corr_spy_hyg, win=HIST_WIN, q=SPIKE_Q)
...
risk4 = spike_flag(eq_corr, win=HIST_WIN, q=SPIKE_Q)


#Risk5 (VVIX)
vix_chg = vix.pct_change()
vix_vov = vix_chg.rolling(20, min_periods=15).std()
risk5 = spike_flag(vix_vov, win=252, q=0.90).reindex(idx).fillna(False)

#Risk6 (Credit spread)
hy = pd.read_csv("HY_OAS.csv", parse_dates=["observation_date"])
...
# --- "Credit is worsening" trigger
HY_LOOKBACK   = 20     # ~1 month
HY_WIDEN_BPS  = 75     # spreads widened by >75 bps over 1 month
HY_LEVEL_BPS  = 350    # only care if spreads already elevated (>350 bps)

hy_widen = hy_oas_bps.diff(HY_LOOKBACK)              # 1-month change in bps
#risk6 = (hy_widen > HY_WIDEN_BPS) & (hy_oas_bps > HY_LEVEL_BPS)
risk6 =  (hy_oas_bps > HY_LEVEL_BPS)


#conditions 
#overlay_on = ((bear_state == 1) & (qqq < ema10) & (vix > VIX_THRESHOLD) & (risk3 | risk4 | risk5 | risk6)).astype(int)
overlay_on = ((vix > VIX_THRESHOLD) & (risk6)).astype(int)
#overlay_on = (risk3 | risk5).astype(int)
